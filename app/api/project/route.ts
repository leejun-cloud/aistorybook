...
// GET  → 로그인한 사용자가 볼 수 있는 프로젝트 목록 (대시보드)
export async function GET() {
  const { uid, master } = await getViewer();
  return NextResponse.json({ projects: await listProjects(uid, master) });
}
...
export async function POST(req: NextRequest) {
  const { uid } = await getViewer();
  const { title, referenceId } = await req.json().catch(() => ({ title: '' }));
  const project = await createProject(title ?? '', uid ?? undefined);
  if (referenceId) {
